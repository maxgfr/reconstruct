...
| `--specs` | flag | off | Also write `SPECS.md` — the same feature PRDs as `--features`, but with each one's `## Source material` section (the embedded source code) stripped: the specs/requirements without the code. Without `--repo`, runs standalone on an existing `--out`. |
