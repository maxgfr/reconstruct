...
| `--orchestrate` | flag | off | Emit the multi-agent orchestration for an existing `--out` into `<out>/orchestration/`: one launchable workflow per ready phase (`enrich-map` \| `review-find` \| `review-verify` \| `adjudicate`), the `agents/<role>.md` dispatch contracts (drafter/finder/verifier/adjudicator) and a sequential `RUNBOOK.md`. Built from the CURRENT worklists (`inventory.json`, `REVIEW.todo.json`, `REVIEW.json`, `VERIFY.todo.json`) with real ids baked in — deterministic and idempotent. Subagents RETURN fragments; the reduce/`--apply` fold stays with the orchestrator. Exit 2 on a missing `--out`, an unknown phase, or a phase whose worklist does not exist yet (the error names the producing command). Reads no repo. See [`references/orchestration.md`](./skills/reconstruct/references/orchestration.md). |
| `--phase <name>` | `enrich-map` \| `review-find` \| `review-verify` \| `adjudicate` | all ready phases | With `--orchestrate`, emit one phase only. |
| `--eco` | flag | off | With `--orchestrate`, emit only `RUNBOOK.md` + `agents/*.md` — the explicit sequential, low-token path. |
| `--list` | flag | off | With `--orchestrate`, print the `{"phases":[...]}` readiness JSON (worklist, item count, producing command per phase) and write nothing. |
...
| **candidates** | [`src/detect/candidates.ts`](./src/detect/candidates.ts) | Framework-agnostic **hints**: candidate files for routes, API surface (tRPC/GraphQL/gRPC/OpenAPI), data model (ORM schemas/models), design-system sources (Tailwind/theme/token configs, global CSS), realtime/auth signals, and entry points — from path + bounded content heuristics. |
